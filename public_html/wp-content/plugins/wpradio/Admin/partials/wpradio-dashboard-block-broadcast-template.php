		...
		<dl class="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-6">
			<div class="sm:col-span-1 flex flex-start">
				<div>
					...
					<dd x-text="$store.server.channelIsOnline(selectedKey) ? $store.server.data.mounts[selectedKey].source_ip : ''"
			...
			<div class="sm:col-span-1">
				...
				<dd x-text="$store.server.channelIsOnline(selectedKey) ? $store.server.data.mounts[selectedKey].server_type : ''"
					class="mt-1 text-sm leading-5 text-gray-900">
				</dd>
			</div>
			<div class="sm:col-span-1">
				...
				<dd x-text="$store.server.channelIsOnline(selectedKey) ? Math.ceil($store.server.data.mounts[selectedKey].incoming_bitrate / 1000) + 'Kbps': ''"
					class="mt-1 text-sm leading-5 text-gray-900">
				</dd>
			</div>
			<div class="sm:col-span-1">
			...
			<div class="sm:col-span-2">
				...
				<dd
					x-text="$store.server.channelIsOnline(selectedKey) ? $store.server.formatIcecastDate($store.server.data.mounts[selectedKey].stream_start) : ''"
					class="mt-1 text-sm leading-5 text-gray-900">
				</dd>
			</div>
			...
			<div x-data="{ open: false, inputOpen: false, loading:false }"
				...
				<dd class="mt-2 text-sm leading-5 text-gray-900">
					<div x-show="!inputOpen"
						 x-text="($store.server.channelIsOnline(selectedKey) && $store.server.data.mounts[selectedKey].current_song !== '') ? $store.server.data.mounts[selectedKey].current_song : '--'"
					...
					<div x-show="inputOpen" class="mt-1 flex rounded-md shadow-sm items-center">
						<div class="relative flex-grow focus-within:z-10">
							<input maxlength="26" x-ref="currentSongInput"
								   :placeholder="$store.server.channelIsOnline(selectedKey) ? $store.server.data.mounts[selectedKey].current_song : ''"
								   id="email"
			...
			<div class="sm:col-span-2 flex">
				...
				<span
					...
					<audio id="wpradio-streamplayer"
						   ...
						   x-bind:type="( $store.server.channelIsOnline(selectedKey) && !$store.account.data.subscription.protected_stream ) ? $store.server.data.mounts[selectedKey].server_type : ''">
