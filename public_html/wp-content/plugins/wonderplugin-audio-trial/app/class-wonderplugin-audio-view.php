...
class WonderPlugin_Audio_View {
	...
	function add_metaboxes() {
		add_meta_box('overview_features', __('Wonder Audio Player Features', 'wonderplugin_audio'), array($this, 'show_features'), 'wonderplugin_audio_overview', 'features', '');
	...
	function print_overview() {
		...
		<h2><?php echo __( 'Wonder Audio Player', 'wonderplugin_audio' ) . ( (WONDERPLUGIN_AUDIO_VERSION_TYPE == "C") ? " Pro" : ( (WONDERPLUGIN_AUDIO_VERSION_TYPE == "L") ? " Lite" : " Trial" ) ) . " Version " . WONDERPLUGIN_AUDIO_VERSION; ?> </h2>
	...
	?>
		...
		$livecacheinterval = $settings['livecacheinterval'];
		?>
		...
		<tr>
			<th>AJAX option</th>
			<td><label><input name='serviceajaxverifynonce' type='checkbox' id='serviceajaxverifynonce' <?php echo ($serviceajaxverifynonce == 1) ? 'checked' : ''; ?> /> Verify nonce for Ajax service</label>
			</td>
		</tr>
		
		<tr>
			<th>Items per Page</th>
			<td><label><input name='itemsperpage' type='number' id='itemsperpage' value="<?php echo $itemsperpage; ?>" class="small-text" /></label>
			</td>
		</tr>
		...
		<tr>
			<th>Cache live streaming title information for </th>
			<td><label><input name='livecacheinterval' type='number' id='livecacheinterval' value="<?php echo $livecacheinterval; ?>" class="small-text"/> seconds</label>
			</td>
		</tr>
	...
	function print_register() {
		...
		function validateLicenseForm() {
			...
			if (jQuery.trim(jQuery("#wonderplugin-audio-key").val()).length <= 0)
			{
	...
	function save_item_post($item_post) {
		...
		{
			echo "<h2>Wonder Audio Player</h2>";
	...
	?>
		...
		<ul class="wonderplugin-tabs-horizontal" id="wonderplugin-popup-display-panels">
        	...
        	<p><label><input type="radio" name="keepid" value=0>Append to the existing audio player list </label></p>
			...
			<p><label><input type="checkbox" name="exportplaylist" value=1>Only export playlist to an XML file</label></p>
