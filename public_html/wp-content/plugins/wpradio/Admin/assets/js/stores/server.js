...
Alpine.store('server', {
  ...
  recorderStatus: (channel) => {
    if (serverStore().data === null || !serverStore().data.mounts.hasOwnProperty(channel)) {
      return '';
    }
    if (accountStore().data.subscription.podcasts_recorder === null) {
      return __('Upgrade your plan to unlock');
    }
    let podcastDuration = accountStore().data.subscription.podcasts_recorder.duration;

    let broadcastStart = parse(serverStore().data.mounts[channel].stream_start, 'dd/MMM/y:HH:mm:ss XX', new Date());
    let recorderFinishTime = parse(serverStore().data.mounts[channel].stream_start, 'dd/MMM/y:HH:mm:ss XX', new Date());
    recorderFinishTime = add(recorderFinishTime, {
      years: 0,
      months: 0,
      weeks: 0,
      days: 0,
      hours: podcastDuration,
      minutes: 0,
      seconds: 0,
    });
    if (isBefore(new Date(), recorderFinishTime)) {
      return __('Recording');
    }
  ...
  onlineChannelsText: () => {
    if (accountStore().data === null || serverStore().data === null || Object.keys(serverStore().data.mounts).length === 0) {
      return 'Off Air';
    }
    if (serverStore().data !== null && Object.keys(serverStore().data.mounts).length === Object.keys(accountStore().data.channels).length && Object.keys(accountStore().data.channels).length === 1) {
      return 'On Air';
    }
    if (serverStore().data !== null) {
      return Object.keys(serverStore().data.mounts).length + ' / ' + Object.keys(accountStore().data.channels).length;
    }
  },
  onlineChannels: () => {
    if (accountStore().data === null || serverStore().data === null || Object.keys(serverStore().data.mounts).length === 0) {
    ...
    if (serverStore().data !== null && Object.keys(serverStore().data.mounts).length === Object.keys(accountStore().data.channels).length) {
      return 'all';
    }
    if (serverStore().data !== null && Object.keys(serverStore().data.mounts).length !== Object.keys(accountStore().data.channels).length) {
      return 'some';
  ...
  channelIsOnline: (channelKey) => {
    return (accountStore().data !== null && serverStore().data !== null && serverStore().data.mounts.hasOwnProperty(channelKey));
  },
  formatIcecastDate: (dateString) => {
    return format(parse(dateString, 'dd/MMM/y:HH:mm:ss XX', new Date()), 'MMM do yyyy, HH:mm');
  },
  actions: {
    startServer: () => {
    ...
    dropSource: (mountpoint) => {
      return new Promise(function (resolve, reject) {
        StreamingServer().get('killsource.xsl?mount=/' + mountpoint)
          .then(function (message) {
            updateServerStatus().finally(function () {
    ...
    updateMetadata: (mountpoint, title) => {
      return new Promise(function (resolve, reject) {
        ...
        StreamingServer().get('metadata.xsl?mode=updinfo&charset=UTF-8&mount=/' + mountpoint + '&song=' + encodeURIComponent(title))
          .then(function (message) {
            updateServerStatus().finally(function () {
    ...
    if (Alpine.store('account').data !== null && Object.keys(Alpine.store('account').data.channels).length > 1) {
      Alpine.store('account').data.channels = _.indexBy(
        _.sortBy(unproxyfiedObject, function(channel) {
          return Alpine.store('server').data.mounts.hasOwnProperty(channel.streaming_server_mountpoint) ? 0 : 1;
        }),
