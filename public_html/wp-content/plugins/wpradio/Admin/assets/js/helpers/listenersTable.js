  ...
  return {
    ...
    loadListeners(mountpoint) {
      ...
      if (serverStore().channelIsOnline(mountpoint) && properties.currentAction === null) {
        ...
        StreamingServer().get('listclients-json.xsl?mount=/' + mountpoint)
          .then(function (response) {

            let listenersArray = response.data['/' + mountpoint];
            ...
            if (listenersArray.length === 0) {
              ...
              properties.listeners = _.map(listenersArray, function (listener) {
                let userAgent = new UAParser(listener.agent);
                return {
                  ip: listener.ip,
                  id: listener.id,
                  duration: Math.ceil(listener.connected / 60) + ' ' + __('Minutes'),
                  lag: listener.lag + ' ' + __('Bytes'),
    ...
    kickListener(mountpoint, listenerId) {
      ...
      if (serverStore().channelIsOnline(mountpoint)) {
        StreamingServer().get('killclient.xsl?mount=/' + mountpoint + '&id=' + listenerId)
          .then(function (response) {
            setTimeout(function () {
